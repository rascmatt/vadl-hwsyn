...
class DECODE extends Module {
  ...
  when ((io.DECODE_en_in).asBool) {
    writeX0_enable := and(neq(io.read_readMEM1_result_in.data.asUInt(11, 7), "h0".U(5.W)), or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(is_LB, is_JAL), is_SRA), is_LHU), is_LBU), is_ORI), is_SLT), is_XOR), is_SLTI), is_SRLI), is_LW), is_SLL), is_SRL), is_JALR), is_LUI), is_ANDI), is_XORI), is_OR), is_LH), is_SLLI), is_SUB), is_SLTIU), is_SLTU), is_AND), is_ADD), is_AUIPC), is_ADDI), is_SRAI))
  }
  when ((io.DECODE_en_in).asBool) {
    is_JALRBNEBGEUBGEJALBLTUBEQBLT := or(or(or(or(or(or(or(is_JALR, is_BNE), is_BGEU), is_BGE), is_JAL), is_BLTU), is_BEQ), is_BLT)
  }
  when ((io.DECODE_en_in).asBool) {
    is_S := sig_147
  }
  ...
  when ((io.DECODE_en_in).asBool) {
    bin_is_BEQ_is_BNE_is_BGE_is_BG := OHToUInt(Cat(is_JALR, is_JAL, is_BLTU, is_BLT, is_BGEU, is_BGE, is_BNE, is_BEQ))
  }
  when ((io.DECODE_en_in).asBool) {
    sel_writePC0_value := OHToUInt(Cat(is_JALR, is_JAL, sig_152))
  }
  when ((io.DECODE_en_in).asBool) {
    sel_readMEM0_words := OHToUInt(Cat(or(is_LH, is_LHU), is_LW, or(is_LB, is_LBU)))
  }
  when ((io.DECODE_en_in).asBool) {
    sel_writeMEM0_words := OHToUInt(Cat(is_SW, is_SH, is_SB))
  }
  when ((io.DECODE_en_in).asBool) {
    sel_writeX0_value := OHToUInt(Cat(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(is_SRL, is_JALR), is_LUI), is_JAL), is_ANDI), is_SRA), is_XORI), is_OR), is_ORI), is_SLT), is_SLLI), is_XOR), is_SLTI), is_SUB), is_SLTIU), is_SLTU), is_SRLI), is_AND), is_ADD), is_AUIPC), is_ADDI), is_SLL), is_SRAI), is_LW, is_LHU, is_LH, is_LBU, is_LB))
  }
  when ((io.DECODE_en_in).asBool) {
    sel_readMEM0_addr := OHToUInt(Cat(or(is_JALR, is_JAL), is_SRAI, is_SRLI, is_SLLI, is_LUI, is_AUIPC, is_SLTIU, is_SLTI, is_XORI, is_ORI, is_ANDI, is_ADDI, is_SRA, is_SRL, is_SLL, is_SLTU, is_SLT, is_XOR, is_OR, is_AND, is_SUB, is_ADD, sig_214, sig_147))
  }
  ...
  rs2_neq_0_and_is_SRLBGEUBGESRA := and(n_166, or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(or(is_SRL, is_BGEU), is_BGE), is_SRA), is_OR), is_SH), is_BLTU), is_BLT), is_SLT), is_XOR), is_BNE), is_SUB), is_SLTU), is_AND), is_ADD), is_SLL), is_SB), is_BEQ), is_SW))
